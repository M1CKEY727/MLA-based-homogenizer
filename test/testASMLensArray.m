...
rate=[d/N,d/N];
area = d*num;           % 光场的面积
lensArray=lensArray([r1,r2],t,d,material,lambda);
%% 生成透镜相位
%[delta_edge,delta_mid,delta_air,t_edge]=lensArray_all([r1,r2],d,t,N,num,'hexagon',0,0.175);
Lens.lensArrayThickFcn(lensArray,N,num,'square',0);
%lensArray.thick_edge=lensArray.thick_edge(:,N/2,:);
%lensArray.thick_air=lensArray.thick_air(:,N/2,:);
subplot(2,2,1)
...
F = transform(afterLens, d*num, lambda, distance, [rate, rate]);
F2=F.*exp(1i*anglePhase([1,0],lambda,area,rate));
F = transform(F2, d*num, lambda, 5*distance, [rate, rate]);
Ia=abs(afterLens).^2;
Ia=gather(Ia);
I = abs(F).^2;
I = gather(I);
